; homework 1 beginner, intermidiate and advanced
; used color instead of lables for pheromone
; needed to make changes to return to nest to make that work
; as color was messing with the conditional
...
to setup
  clear-all  ; clear all at the beginning and the reset-ticks at the end
...
to go
  ...
  [ ifelse return-to-nest? = false
    [look-for-food]    ;; if condition met
    [return-to-nest]   ;; else condition
  ]
  evaporate-pheromone
  tick
end
...
;; create a new procedure to look for food
;;

to look-for-food
  ; adding a sent sensing ahead right and left
  ; if senses phermone go that direction else move radomly as before
  ...
  ; if condition is met --turn toward pheromone
  [ifelse pheromone-right? > pheromone-left?
    [rt 45] ;first codition
    [lt 45] ;else
  ]
  ; else condontion turn random i.e. no pheromoe ahead
  ...
  forward random max-step-size ;moves forward nomater what
]
  if pcolor = green ; if the turtle is located on a green patch
    [
      set pcolor black
      set food-eaten (food-eaten + 1)
      set return-to-nest? true ; if they eat they should return to the nest
      ;set label food-eaten
      ;if (food-eaten > 2) [set color blue]  ; color mapped to food eaten
...
;; create procedure to return to nest

to return-to-nest
  face patch 0 0 fd 1 ; turn toward nest
  set pheromone pheromone + 1 ; incr phero note no = for assignment
  ;set plabel pheromone
  set pcolor 10 + pheromone  ; map color to pheromone level
  ;if pcolor = orange [set return-to-nest? false] ;; assignment without =
  if (distancexy 0 0) < 2 [set return-to-nest? false] ;; for use with color pheronome

end

;; procedure to evaporate pheromone

to evaporate-pheromone
  let x random-float 1 ; let becaues it is not patch or turtule
  ask patches with [pheromone > 0]
  [if x < probability-to-evaporate
    [set pheromone pheromone - 1]
    ;set plabel pheromone
    set pcolor 10 + pheromone  ; map color to pheromone
  ]
...
to set-up-patches
  ;; nest procedure changed to set up patches
  ask patches [
    set pheromone 0  ; at set up pheromone zero for all patches
    set nest? (distancexy 0 0) < 2 ; nest is true under if it is less than 2 from origin
...
following homework from introduction to complexity module 1
